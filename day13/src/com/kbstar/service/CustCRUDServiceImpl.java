...
public class CustCRUDServiceImpl implements CRUDService<String, Cust> {
	...
	public void register(Cust v) throws Exception {
		...
		/**
		 * 여기서 원래 
		 * da.insert(v);로 끝나는 코드로 실행이 가능한데
		 * 테스트를 하던 중 같은 값의 id를 입력하면 
		 * violated .... 라는 에러메세지가 나온다. 
		 * 이러한 오류의 경우에 메세지를 표츌하기 위해서는 
		 * 이곳에서 Exception을 한번 더 잡아주어야 한다. 
		 * 잡아주는 과정에서 중복등록으로 인한 오류라는 메세지를 고객이 보는 화면으로 날려줄 수 있다.
		 */
		...
		try {
			...
			}else throw new Exception("시스템 장애 입니다..");
		}
